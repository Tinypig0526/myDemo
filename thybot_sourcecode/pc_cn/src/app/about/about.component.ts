...
export class AboutComponent implements OnInit, AfterViewInit {
  ...
  url = 'http://192.168.1.13/homepage/backend/web/api/article/get-article';
  ...
  ngOnInit() {
    this.control.getData(this.url).then(data => {
      this.result = data.data;
      for (let i = 0; i < this.result.length; i++) {
        this.result[i].time = this.timestampToTime(this.result[i].create_time);
      }
